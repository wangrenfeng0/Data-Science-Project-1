...
df_beer_stout=beer_imp %>% filter(!is.na(ABV) &!is.na(IBU)) %>% 
  filter(str_detect(Style, regex(str_c('\\b','stout','\\b',sep=''), ignore_case = T))) # Find all Stout style beer

df_beer_lager=beer_imp %>% filter(!is.na(ABV) &!is.na(IBU)) %>% 
  filter(str_detect(Style, regex(str_c('\\b','lager','\\b',sep=''), ignore_case = T))) # Find all Lager style beer
